...
extern "C" {
typedef void* (*armsx2_openemu_metal_object_callback_t)(void* context);
typedef void (*armsx2_openemu_metal_notify_callback_t)(void* context);
}

#if defined(__GNUC__) || defined(__clang__)
#define ARMSX2_OPENEMU_EXPORT __attribute__((visibility("default")))
#else
#define ARMSX2_OPENEMU_EXPORT
#endif

namespace OpenEmuMetalBridge
{
	static void* context = nullptr;
	static armsx2_openemu_metal_object_callback_t get_device = nullptr;
	static armsx2_openemu_metal_object_callback_t get_texture = nullptr;
	static armsx2_openemu_metal_notify_callback_t will_execute = nullptr;
	static armsx2_openemu_metal_notify_callback_t did_execute = nullptr;
	static std::atomic<bool> has_direct_present{false};
...
extern "C" ARMSX2_OPENEMU_EXPORT void armsx2_openemu_set_metal_callbacks(
	void* context,
	armsx2_openemu_metal_object_callback_t get_device,
	armsx2_openemu_metal_object_callback_t get_texture,
	armsx2_openemu_metal_notify_callback_t will_execute,
	armsx2_openemu_metal_notify_callback_t did_execute)
{
	OpenEmuMetalBridge::context = context;
	OpenEmuMetalBridge::get_device = get_device;
	OpenEmuMetalBridge::get_texture = get_texture;
	OpenEmuMetalBridge::will_execute = will_execute;
	OpenEmuMetalBridge::did_execute = did_execute;
	OpenEmuMetalBridge::has_direct_present.store(false, std::memory_order_release);
	OpenEmuMetalBridge::DebugLog("[ARMSX2-libretro] Metal callbacks registered: context=%p device=%p texture=%p will=%p did=%p",
		context, reinterpret_cast<void*>(get_device), reinterpret_cast<void*>(get_texture),
		reinterpret_cast<void*>(will_execute), reinterpret_cast<void*>(did_execute));
}

extern "C" ARMSX2_OPENEMU_EXPORT void* armsx2_openemu_get_metal_device(void)
{
	return OpenEmuMetalBridge::get_device ? OpenEmuMetalBridge::get_device(OpenEmuMetalBridge::context) : nullptr;
}

extern "C" ARMSX2_OPENEMU_EXPORT void* armsx2_openemu_get_metal_texture(void)
{
	return OpenEmuMetalBridge::get_texture ? OpenEmuMetalBridge::get_texture(OpenEmuMetalBridge::context) : nullptr;
}

extern "C" ARMSX2_OPENEMU_EXPORT bool armsx2_openemu_metal_active(void)
{
	return OpenEmuMetalBridge::get_texture != nullptr;
}

extern "C" ARMSX2_OPENEMU_EXPORT void armsx2_openemu_will_execute_metal(void)
{
	if (OpenEmuMetalBridge::will_execute)
		OpenEmuMetalBridge::will_execute(OpenEmuMetalBridge::context);
}

extern "C" ARMSX2_OPENEMU_EXPORT void armsx2_openemu_did_execute_metal(void)
{
	if (OpenEmuMetalBridge::did_execute)
		OpenEmuMetalBridge::did_execute(OpenEmuMetalBridge::context);
}

extern "C" ARMSX2_OPENEMU_EXPORT void armsx2_openemu_mark_metal_presented(void)
{
	OpenEmuMetalBridge::has_direct_present.store(true, std::memory_order_release);
	static std::atomic<u64> present_count{0};
	const u64 count = ++present_count;
	if (count <= 5 || (count % 60) == 0)
		OpenEmuMetalBridge::DebugLog("[ARMSX2-libretro] Direct Metal present marked: %llu", count);
}
...
namespace LibretroCore
{
	...
	static u32 s_frame_width = kFrameWidth;
	static u32 s_frame_height = kFrameHeight;
	static std::vector<u32> s_frame_buffer;
	static std::vector<u32> s_snapshot_buffer;
	static u64 s_present_frame_count = 0;
...
static void FillDiagnosticFrame()
{
	if (LibretroCore::s_present_frame_count <= 5 || LibretroCore::s_present_frame_count == 60)
		log_cb(RETRO_LOG_INFO, "Software frame fallback: diagnostic frame %llu.\n", LibretroCore::s_present_frame_count);

	LibretroCore::s_frame_width = LibretroCore::kFrameWidth;
	LibretroCore::s_frame_height = LibretroCore::kFrameHeight;
	LibretroCore::s_frame_buffer.resize(LibretroCore::kFrameWidth * LibretroCore::kFrameHeight);

	for (u32 y = 0; y < LibretroCore::kFrameHeight; y++)
	{
		for (u32 x = 0; x < LibretroCore::kFrameWidth; x++)
		{
			const u8 r = static_cast<u8>((x + LibretroCore::s_present_frame_count) & 0xff);
			const u8 g = static_cast<u8>((y * 2) & 0xff);
			const u8 b = static_cast<u8>(((x / 32 + y / 32 + LibretroCore::s_present_frame_count / 20) & 1) ? 0x44 : 0xaa);
			LibretroCore::s_frame_buffer[y * LibretroCore::kFrameWidth + x] =
				(static_cast<u32>(r) << 16) | (static_cast<u32>(g) << 8) | static_cast<u32>(b);
		}
	}
}

static bool CaptureSoftwareFrame()
{
	if (!VMManager::HasValidVM())
	{
		if (LibretroCore::s_present_frame_count <= 5 || LibretroCore::s_present_frame_count == 60)
			log_cb(RETRO_LOG_INFO, "Software frame capture skipped: no valid VM, state=%d, mtgs_open=%d.\n",
				static_cast<int>(VMManager::GetState()), MTGS::IsOpen() ? 1 : 0);
		return false;
	}

	u32 width = 0;
	u32 height = 0;
	if (!MTGS::SaveMemorySnapshot(0, 0, true, false, &width, &height, &LibretroCore::s_snapshot_buffer) ||
		width == 0 || height == 0 || LibretroCore::s_snapshot_buffer.empty())
	{
		if (LibretroCore::s_present_frame_count <= 5 || LibretroCore::s_present_frame_count == 60)
		{
			const char* api_name = g_gs_device ? GSDevice::RenderAPIToString(g_gs_device->GetRenderAPI()) : "none";
			log_cb(RETRO_LOG_INFO,
				"Software frame capture failed: state=%d, mtgs_open=%d, gs_device=%d, api=%s, current=%d, size=%ux%u, pixels=%zu.\n",
				static_cast<int>(VMManager::GetState()), MTGS::IsOpen() ? 1 : 0, g_gs_device ? 1 : 0,
				api_name, (g_gs_device && g_gs_device->GetCurrent()) ? 1 : 0, width, height,
				LibretroCore::s_snapshot_buffer.size());
		}
		return false;
	}

	LibretroCore::s_frame_buffer.swap(LibretroCore::s_snapshot_buffer);
	LibretroCore::s_frame_width = width;
	LibretroCore::s_frame_height = height;
	if (LibretroCore::s_present_frame_count <= 5 || LibretroCore::s_present_frame_count == 60)
		log_cb(RETRO_LOG_INFO, "Software frame capture: %ux%u.\n", width, height);
	return true;
}
...
bool LibretroCore::InitializeConfig()
{
	...
	// Libretro-core overrides: renderer selection, and SDL input/audio
	// replaced by the libretro paths.
	{
		...
		s_base_settings->SetIntValue("EmuCore/GS", "Renderer",
			...
			static_cast<int>(GSRendererType::Metal));
		OpenEmuMetalBridge::DebugLog("[ARMSX2-libretro] Default renderer set to Metal.");
#else
...
void LibretroCore::CPUThreadMain(VMBootParameters initial_params)
{
	...
	OpenEmuMetalBridge::DebugLog("[ARMSX2-libretro] CPU thread started.");
	...
	if (!VMManager::Internal::CPUThreadInitialize())
	{
		...
		OpenEmuMetalBridge::DebugLog("[ARMSX2-libretro] CPUThreadInitialize failed.");
	...
	OpenEmuMetalBridge::DebugLog("[ARMSX2-libretro] CPUThreadInitialize succeeded; boot file=%s.",
		...
		switch (state)
		{
			...
			case VMState::Shutdown:
			{
				if (pending_boot.has_value())
				{
					...
					OpenEmuMetalBridge::DebugLog("[ARMSX2-libretro] VMManager::Initialize starting.");
					Error boot_error;
					const VMBootResult br = VMManager::Initialize(bp, &boot_error);
					std::fprintf(stderr, "[libretro] CPU thread: Initialize -> %d\n", (int)br);
					OpenEmuMetalBridge::DebugLog("[ARMSX2-libretro] VMManager::Initialize result=%d error=%s.",
						static_cast<int>(br), boot_error.GetDescription().c_str());
...
static struct retro_core_option_v2_definition kOptionDefinitions[] = {
	{"armsx2_renderer", "GS Renderer (restart)", "GS Renderer (restart)",
		"Renderer used by the PS2 GS. On macOS, Metal is the native ARM path used for readback presentation.",
		nullptr, "video",
		{{"Vulkan", nullptr}, {"Metal", nullptr}, {"Software", nullptr}, {nullptr, nullptr}},
#ifdef ENABLE_VULKAN
		"Vulkan"
#elif defined(__APPLE__)
		"Metal"
...
static struct retro_variable kCoreVariables[] = {
	{"armsx2_renderer", "GS renderer (restart); Vulkan|Metal|Software"},
...
#ifdef __APPLE__
			else if (!std::strcmp(var.value, "Metal"))
			{
				s_base_settings->SetIntValue("EmuCore/GS", "Renderer", static_cast<int>(GSRendererType::Metal));
				OpenEmuMetalBridge::DebugLog("[ARMSX2-libretro] Core option renderer set to Metal.");
			}
#endif
		}
...
{
	OpenEmuMetalBridge::DebugLog("[ARMSX2-libretro] retro_load_game starting path=%s.",
	...
	OpenEmuMetalBridge::DebugLog("[ARMSX2-libretro] retro_load_game returning true; cpu_thread_initialized=1.");
	return true;
}
...
{
	input_poll_cb();
	static u64 run_count = 0;
	run_count++;
	if (run_count <= 5 || run_count == 30 || run_count == 60 || (run_count % 300) == 0)
	{
		OpenEmuMetalBridge::DebugLog("[ARMSX2-libretro] retro_run #%llu state=%d valid_vm=%d mtgs_open=%d gs_device=%d direct_metal=%d.",
			run_count,
			static_cast<int>(VMManager::GetState()),
			VMManager::HasValidVM() ? 1 : 0,
			MTGS::IsOpen() ? 1 : 0,
			g_gs_device ? 1 : 0,
			OpenEmuMetalBridge::has_direct_present.load(std::memory_order_acquire) ? 1 : 0);
	}
...
#endif
	{
		if (OpenEmuMetalBridge::has_direct_present.load(std::memory_order_acquire))
		{
			video_cb(nullptr, LibretroCore::s_frame_width, LibretroCore::s_frame_height, 0);
		}
		else if (OpenEmuMetalBridge::get_texture)
		{
			LibretroCore::s_present_frame_count++;
			if (LibretroCore::s_present_frame_count <= 5 || (LibretroCore::s_present_frame_count % 60) == 0)
			{
				OpenEmuMetalBridge::DebugLog("[ARMSX2-libretro] No direct Metal draw yet; suppressing CPU fallback frame %llu.",
					LibretroCore::s_present_frame_count);
			}
			video_cb(nullptr, LibretroCore::s_frame_width, LibretroCore::s_frame_height, 0);
		}
		else
		{
			...
			if (LibretroCore::s_present_frame_count <= 5 || (LibretroCore::s_present_frame_count % 60) == 0)
			{
				OpenEmuMetalBridge::DebugLog("[ARMSX2-libretro] No direct Metal present yet; using CPU fallback frame %llu.",
					LibretroCore::s_present_frame_count);
			}
			if (!CaptureSoftwareFrame())
				FillDiagnosticFrame();

			static u32 last_geometry_width = 0;
			static u32 last_geometry_height = 0;
			if (LibretroCore::s_frame_width != last_geometry_width || LibretroCore::s_frame_height != last_geometry_height)
			{
				last_geometry_width = LibretroCore::s_frame_width;
				last_geometry_height = LibretroCore::s_frame_height;
				retro_game_geometry geometry = {};
				geometry.base_width = LibretroCore::s_frame_width;
				geometry.base_height = LibretroCore::s_frame_height;
			...
			video_cb(LibretroCore::s_frame_buffer.data(), LibretroCore::s_frame_width,
				LibretroCore::s_frame_height, LibretroCore::s_frame_width * sizeof(u32));
		}
	}
