...
namespace VMManager
{
	static void ARMSX2VMInitLog(const char* format, ...)
	{
		FILE* fp = std::fopen("/tmp/openemu-armsx2-metal.log", "a");
		if (!fp)
			return;
...
void VMManager::ApplyGameFixes()
{
	...
	if (!game)
	{
		ARMSX2VMInitLog("[ARMSX2-VMManager] GameDB: no entry for serial '%s'; automatic game fixes were not applied.", s_disc_serial.c_str());
		return;
	}

	game->applyGameFixes(EmuConfig, EmuConfig.EnableGameFixes);
	game->applyGSHardwareFixes(EmuConfig.GS);
	ARMSX2VMInitLog("[ARMSX2-VMManager] GameDB: applied '%s' manual_hacks=%d auto_flush=%d half_pixel=%d hw_download=%d skip_handler=%d blend=%d.",
		s_disc_serial.c_str(), static_cast<int>(EmuConfig.GS.ManualUserHacks),
		...
		static_cast<int>(EmuConfig.GS.GetSkipCountFunctionId), static_cast<int>(EmuConfig.GS.AccurateBlendingUnit));
...
VMBootResult VMManager::Initialize(const VMBootParameters& boot_params, Error* error)
{
	...
	ARMSX2VMInitLog("[ARMSX2-VMManager] Opening GS.");
	...
	if (!s_gs_open_on_initialize && !MTGS::WaitForOpen())
	{
		...
		ARMSX2VMInitLog("[ARMSX2-VMManager] GS open failed.");
		Error::SetString(error, TRANSLATE_STR("VMManager", "Failed to initialize GS."));
		return VMBootResult::StartupFailure;
	}
	ARMSX2VMInitLog("[ARMSX2-VMManager] GS opened: already_open=%d.", static_cast<int>(s_gs_open_on_initialize));
	...
	ARMSX2VMInitLog("[ARMSX2-VMManager] Opening SPU2.");
	if (!SPU2::Open())
	{
		ARMSX2VMInitLog("[ARMSX2-VMManager] SPU2 open failed.");
		Error::SetString(error, TRANSLATE_STR("VMManager", "Failed to initialize SPU2."));
		return VMBootResult::StartupFailure;
	}
	ARMSX2VMInitLog("[ARMSX2-VMManager] SPU2 opened.");
	ScopedGuard close_spu2(&SPU2::Close);


	Console.WriteLn("Initializing Pad...");
	ARMSX2VMInitLog("[ARMSX2-VMManager] Initializing Pad.");
	if (!Pad::Initialize())
	{
		ARMSX2VMInitLog("[ARMSX2-VMManager] Pad init failed.");
		Error::SetString(error, TRANSLATE_STR("VMManager", "Failed to initialize PAD."));
		return VMBootResult::StartupFailure;
	}
	ARMSX2VMInitLog("[ARMSX2-VMManager] Pad initialized.");
	WarnAboutUnconfiguredController();
	ScopedGuard close_pad = &Pad::Shutdown;

	Console.WriteLn("Initializing SIO2...");
	ARMSX2VMInitLog("[ARMSX2-VMManager] Initializing SIO2.");
	if (!g_Sio2.Initialize())
	{
		ARMSX2VMInitLog("[ARMSX2-VMManager] SIO2 init failed.");
		Error::SetString(error, TRANSLATE_STR("VMManager", "Failed to initialize SIO2."));
		return VMBootResult::StartupFailure;
	}
	ARMSX2VMInitLog("[ARMSX2-VMManager] SIO2 initialized.");
	ScopedGuard close_sio2 = []() {
		g_Sio2.Shutdown();
	};

	Console.WriteLn("Initializing SIO0...");
	ARMSX2VMInitLog("[ARMSX2-VMManager] Initializing SIO0.");
	if (!g_Sio0.Initialize())
	{
		ARMSX2VMInitLog("[ARMSX2-VMManager] SIO0 init failed.");
		Error::SetString(error, TRANSLATE_STR("VMManager", "Failed to initialize SIO0."));
		return VMBootResult::StartupFailure;
	}
	ARMSX2VMInitLog("[ARMSX2-VMManager] SIO0 initialized.");
	ScopedGuard close_sio0 = []() {
		g_Sio0.Shutdown();
	};

	Console.WriteLn("Opening DEV9...");
	ARMSX2VMInitLog("[ARMSX2-VMManager] Opening DEV9.");
	if (DEV9init() != 0 || DEV9open() != 0)
	{
		ARMSX2VMInitLog("[ARMSX2-VMManager] DEV9 open failed.");
		Error::SetString(error, TRANSLATE_STR("VMManager", "Failed to initialize DEV9."));
		return VMBootResult::StartupFailure;
	}
	ARMSX2VMInitLog("[ARMSX2-VMManager] DEV9 opened.");
	...
	ARMSX2VMInitLog("[ARMSX2-VMManager] Opening USB.");
	if (!USBopen())
	{
		ARMSX2VMInitLog("[ARMSX2-VMManager] USB open failed.");
		Error::SetString(error, TRANSLATE_STR("VMManager", "Failed to initialize USB."));
		return VMBootResult::StartupFailure;
	}
	ARMSX2VMInitLog("[ARMSX2-VMManager] USB opened.");
	ScopedGuard close_usb = []() { USBclose(); };

	Console.WriteLn("Opening FW...");
	ARMSX2VMInitLog("[ARMSX2-VMManager] Opening FW.");
	if (FWopen() != 0)
	{
		ARMSX2VMInitLog("[ARMSX2-VMManager] FW open failed.");
		Error::SetString(error, TRANSLATE_STR("VMManager", "Failed to initialize FW."));
		return VMBootResult::StartupFailure;
	}
	ARMSX2VMInitLog("[ARMSX2-VMManager] FW opened.");
	...
	ARMSX2VMInitLog("[ARMSX2-VMManager] VM subsystems initialized in %.2f ms.", init_timer.GetTimeMilliseconds());
	...
	ARMSX2VMInitLog("[ARMSX2-VMManager] Initialize returning StartupSuccess.");
