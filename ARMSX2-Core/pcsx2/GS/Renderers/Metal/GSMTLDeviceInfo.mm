...
static id<MTLLibrary> loadMainLibrary(id<MTLDevice> dev, NSString* name)
{
	...
	NSError* error = nil;
	id<MTLLibrary> library = path ? [dev newLibraryWithFile:path error:&error] : nullptr;
	FILE* file = std::fopen("/tmp/openemu-armsx2-metal.log", "a");
	if (file)
	{
		std::fprintf(file, "[ARMSX2-GSMTLDevice] load %s.metallib path=%s ok=%d error=%s\n",
			[name UTF8String], path ? [path fileSystemRepresentation] : "(null)", library != nil,
