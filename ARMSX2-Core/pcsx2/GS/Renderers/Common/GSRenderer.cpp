...
#ifdef __APPLE__
extern "C" bool armsx2_openemu_metal_active(void);
#endif
...
static void ARMSX2GSRendererLog(const char* fmt, ...)
{
...
void GSRenderer::EndPresentFrame()
{
	...
	// The OpenEmu Metal bridge has no PCSX2 ImGui context. Rendering the
	// fullscreen UI/OSD here blocks before GSDeviceMTL::EndPresent can submit
	// its first command buffer, leaving the host with a permanent black frame.
#ifdef __APPLE__
	const bool openemu_metal = armsx2_openemu_metal_active();
#else
	constexpr bool openemu_metal = false;
#endif
	if (!openemu_metal)
	{
		FullscreenUI::Render();
...
void GSRenderer::VSync(u32 field, bool registers_written, bool idle_frame)
{
	...
	else
	{
		...
		static u64 openemu_renderer_present_count = 0;
		openemu_renderer_present_count++;
		if (openemu_renderer_present_count <= 5 || openemu_renderer_present_count == 30 ||
			openemu_renderer_present_count == 60 || (openemu_renderer_present_count % 300) == 0)
		{
			ARMSX2GSRendererLog("[ARMSX2-GSRenderer] present frame=%llu current=%d blank=%d skip_frame=%d registers=%d fb_sprite=%d.",
				openemu_renderer_present_count,
				current != nullptr,
				blank_frame,
				skip_frame,
