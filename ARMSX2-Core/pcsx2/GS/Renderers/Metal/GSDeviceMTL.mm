...
extern "C" void* armsx2_openemu_get_metal_device(void);
extern "C" void* armsx2_openemu_get_metal_texture(void);
extern "C" void armsx2_openemu_will_execute_metal(void);
extern "C" void armsx2_openemu_did_execute_metal(void);
extern "C" void armsx2_openemu_mark_metal_presented(void);

static void ARMSX2OpenEmuMetalLog(const char* fmt, ...)
{
	FILE* file = std::fopen("/tmp/openemu-armsx2-metal.log", "a");
	if (!file)
		return;
...
MRCOwned<id<MTLFunction>> GSDeviceMTL::LoadShader(NSString* name)
{
	...
	if (err) [[unlikely]]
	{
		...
		ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] %s", [msg UTF8String]);
		Console.Error("%s", [msg UTF8String]);
		pxFailRel([msg UTF8String]);
	}
	if (!fn)
		ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Shader %s returned nil without NSError.", [name UTF8String]);
	return fn;
}

MRCOwned<id<MTLFunction>> GSDeviceMTL::LoadShader(NSString* name, NSString* fallback_name)
{
	NSError* err = nil;
	MRCOwned<id<MTLFunction>> fn = MRCTransfer([m_dev.shaders newFunctionWithName:name constantValues:m_fn_constants error:&err]);
	if (fn)
	...
	ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Shader %s unavailable, trying fallback %s: %s",
		[name UTF8String], [fallback_name UTF8String], err ? [[err localizedDescription] UTF8String] : "no NSError");

	fn = LoadShader(fallback_name);
	if (fn)
		ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Shader fallback loaded: %s -> %s", [name UTF8String], [fallback_name UTF8String]);
	return fn;
}
...
MRCOwned<id<MTLRenderPipelineState>> GSDeviceMTL::MakePipeline(MTLRenderPipelineDescriptor* desc, id<MTLFunction> vertex, id<MTLFunction> fragment, NSString* name)
{
	...
	if (!res)
		ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Render pipeline %s returned nil without NSError.", [name UTF8String]);
	return res;
}
...
MRCOwned<id<MTLComputePipelineState>> GSDeviceMTL::MakeComputePipeline(id<MTLFunction> compute, NSString* name)
{
	...
	if (!res)
		ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Compute pipeline %s returned nil without NSError.", [name UTF8String]);
	return res;
}
...
static void setFnConstantB(MTLFunctionConstantValues* fc, bool value, NSUInteger constant)
{
	[fc setConstantValue:&value type:MTLDataTypeBool atIndex:constant];
}
...
static void setFnConstantI(MTLFunctionConstantValues* fc, unsigned int value, NSUInteger constant)
{
	[fc setConstantValue:&value type:MTLDataTypeUInt atIndex:constant];
}
...
{ @autoreleasepool {
	ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Create entered.");
	if (!GSDevice::Create(vsync_mode, allow_present_throttle))
		return false;

	if (void* openemu_device = armsx2_openemu_get_metal_device())
	{
		m_dev = GSMTLDevice(MRCRetain((__bridge id<MTLDevice>)openemu_device));
		Console.WriteLn("Metal: Using OpenEmu Metal device");
		ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Using OpenEmu Metal device.");
	}
	else
	{
		ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] OpenEmu Metal device unavailable; using adapter/default path.");
	}
	...
	if (m_dev.IsOk() && m_queue)
	{
		...
		{
			ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] AcquireWindow(true) failed.");
			return false;
		}
		ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] AcquireWindow succeeded: type=%d size=%ux%u scale=%f.",
			static_cast<int>(m_window_info.type), m_window_info.surface_width, m_window_info.surface_height,
		...
		ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] AttachSurface finished: layer=%d view=%d.",
			m_layer != nil, m_view != nil);
	...
	{
		ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Device invalid or command queue missing: device_ok=%d queue=%d.",
			m_dev.IsOk(), m_queue != nil);
		return false;
	}
	...
	ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Features initialized: shader_version=%s max_tex=%d fbfetch=%d primid=%d rov=%d.",
		to_string(m_dev.features.shader_version), m_max_texture_size, m_features.framebuffer_fetch,
		m_features.primitive_id, m_features.rov);
	...
	ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Basic Metal resources initialized.");
	...
	if (m_features.cas_sharpening)
	{
		...
		ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] CAS pipelines initialized.");
	}
	else
	{
		ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] CAS pipelines skipped for OpenEmu shader-library compatibility.");
	}
	ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Compute pipelines initialized.");
	...
	ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Samplers initialized.");
	...
	ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Depth/stencil states initialized.");
	...
	ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Hardware vertex shaders initialized.");
	...
	ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Fixed-function conversion pipelines initialized.");
	...
	ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Interlace pipelines initialized.");
	...
	for (u32 i = 0; i < ShaderConvertSelector::NUM_TOTAL_SHADERS; i++)
	{
		...
		else
		{
			...
			pdesc.depthAttachmentPixelFormat = shader.Float32Output() ? ConvertPixelFormat(GSTexture::Format::DepthStencil) : MTLPixelFormatInvalid;
		}
		...
		// Compatibility for reused PCSX2 metallibs: clear vertex-shader legacy constants before convert shaders.
		setFnConstantB(m_fn_constants, shader.Biln(),        0);
		setFnConstantB(m_fn_constants, shader.DepthOutput(), 1);
		setFnConstantB(m_fn_constants, shader.DepthOutput(), 2);
		MRCOwned<id<MTLFunction>> ps;
		if (shader.Shader() == ShaderConvert::RGB5A1_TO_16_BITS)
			ps = LoadShader(shader_name, @"ps_convert_rgba8_16bits");
		else if (shader.Shader() == ShaderConvert::DEPTH32_TO_16_BITS || shader.Shader() == ShaderConvert::DEPTH32_TO_32_BITS)
			ps = LoadShader(shader_name, @"ps_convert_float32_32bits");
		else if (shader.Shader() == ShaderConvert::DEPTH32_TO_RGBA8 || shader.Shader() == ShaderConvert::DEPTH32_TO_RGB8)
			ps = LoadShader(shader_name, @"ps_convert_float32_rgba8");
		else if (shader.Shader() == ShaderConvert::DEPTH16_TO_RGB5A1)
			ps = LoadShader(shader_name, @"ps_convert_float16_rgb5a1");
		else if (shader.Shader() == ShaderConvert::RGBA8_TO_DEPTH32)
			ps = LoadShader(shader_name, @"ps_convert_rgba8_float32");
		else if (shader.Shader() == ShaderConvert::RGBA8_TO_DEPTH24)
			ps = LoadShader(shader_name, @"ps_convert_rgba8_float24");
		else if (shader.Shader() == ShaderConvert::RGBA8_TO_DEPTH16)
			ps = LoadShader(shader_name, @"ps_convert_rgba8_float16");
		else if (shader.Shader() == ShaderConvert::RGB5A1_TO_DEPTH16)
			ps = LoadShader(shader_name, @"ps_convert_rgb5a1_float16");
		else if (shader.Shader() == ShaderConvert::DEPTH32_TO_DEPTH24)
			ps = LoadShader(shader_name, @"ps_convert_float32_float24");
		else
			ps = LoadShader(shader_name);
		m_convert_pipeline[shader.Index()] = MakePipeline(pdesc, vs_convert, ps, name);
	}
	ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Convert pipelines initialized.");
	...
	ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Present pipelines initialized.");
	...
	ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Merge pipelines initialized.");
	pdesc.colorAttachments[0].writeMask = MTLColorWriteMaskAll;
	...
	ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] ImGui pipeline initialized.");

	[initCommands commit];
	ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Create returning true.");
	return true;
}}
...
static bool s_openemu_present_encoding = false;
static u64 s_openemu_present_frame = 0;
static u64 s_openemu_present_draws = 0;
...
{ @autoreleasepool {
	...
	{
		if (void* openemu_texture = armsx2_openemu_get_metal_texture())
		{
			static bool logged_openemu_target = false;
			static u64 openemu_present_count = 0;
			openemu_present_count++;
			if (!logged_openemu_target)
			{
				id<MTLTexture> texture = (__bridge id<MTLTexture>)openemu_texture;
				Console.WriteLn("Metal: Presenting to OpenEmu texture (%lux%lu)",
					static_cast<unsigned long>([texture width]),
					static_cast<unsigned long>([texture height]));
				ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] Presenting to OpenEmu texture %lux%lu fmt=%lu.",
					static_cast<unsigned long>([texture width]),
					static_cast<unsigned long>([texture height]),
					static_cast<unsigned long>([texture pixelFormat]));
				logged_openemu_target = true;
			}
			else if (openemu_present_count <= 5 || (openemu_present_count % 60) == 0)
			{
				Console.WriteLn("Metal: OpenEmu present frame %llu", openemu_present_count);
				ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] OpenEmu present frame %llu.", openemu_present_count);
			}

			armsx2_openemu_will_execute_metal();
			m_current_drawable = nullptr;
			[m_pass_desc colorAttachments][0].texture = (__bridge id<MTLTexture>)openemu_texture;
			...
			s_openemu_present_encoding = true;
			s_openemu_present_frame = openemu_present_count;
			s_openemu_present_draws = 0;
			return PresentResult::OK;
		}
	}
	static u64 no_openemu_texture_count = 0;
	no_openemu_texture_count++;
	if (no_openemu_texture_count <= 5 || (no_openemu_texture_count % 60) == 0)
		ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] No OpenEmu texture in DoBeginPresent; count=%llu.", no_openemu_texture_count);
...
{ @autoreleasepool {
	...
	EndRenderPass();
	MRCOwned<id<MTLCommandBuffer>> openemu_present_command_buffer;
	if (!m_current_drawable)
		openemu_present_command_buffer = MRCRetain(m_current_render_cmdbuf.Get());
	if (m_current_drawable)
	{
	...
	FlushEncoders();
	if (openemu_present_command_buffer)
	{
		[openemu_present_command_buffer.Get() waitUntilCompleted];
		const MTLCommandBufferStatus status = [openemu_present_command_buffer.Get() status];
		NSError* error = [openemu_present_command_buffer.Get() error];
		if (s_openemu_present_frame <= 5 || (s_openemu_present_frame % 60) == 0 || status != MTLCommandBufferStatusCompleted || error)
		{
			ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] OpenEmu present completed frame=%llu draws=%llu status=%lu error=%s.",
				s_openemu_present_frame,
				s_openemu_present_draws,
				static_cast<unsigned long>(status),
				error ? [[error localizedDescription] UTF8String] : "");
		}
		s_openemu_present_encoding = false;
	}
	if (openemu_present_command_buffer)
		armsx2_openemu_did_execute_metal();
	FrameCompleted();
	m_current_drawable = nullptr;
	if (m_capture_start_frame)
	{
...
void GSDeviceMTL::DrawStretchRect(const GSVector4& sRect, const GSVector4& dRect, const GSVector2& ds)
{
	...
	if (s_openemu_present_encoding)
	{
		if (s_openemu_present_draws == 0)
			armsx2_openemu_mark_metal_presented();
		s_openemu_present_draws++;
		if (s_openemu_present_frame <= 5 || (s_openemu_present_frame % 60) == 0)
		{
			ARMSX2OpenEmuMetalLog("[ARMSX2-GSDeviceMTL] OpenEmu present draw frame=%llu draw=%llu s=(%.1f,%.1f,%.1f,%.1f) d=(%.1f,%.1f,%.1f,%.1f) ds=(%.1f,%.1f).",
				s_openemu_present_frame,
				s_openemu_present_draws,
				static_cast<double>(sRect.x),
				static_cast<double>(sRect.y),
				static_cast<double>(sRect.z),
				static_cast<double>(sRect.w),
				static_cast<double>(dRect.x),
				static_cast<double>(dRect.y),
				static_cast<double>(dRect.z),
				static_cast<double>(dRect.w),
				static_cast<double>(ds.x),
				static_cast<double>(ds.y));
		}
	}
...
void GSDeviceMTL::MRESetHWPipelineState(GSHWDrawConfig::VSSelector vssel, GSHWDrawConfig::PSSelector pssel, GSHWDrawConfig::BlendState blend, GSHWDrawConfig::ColorMaskSelector cms)
{
	...
	// Vertex and pixel shader selectors are independent. In particular, games
	// such as Gran Turismo 4 legitimately use different FST values between the
	// stages. Reusing the pixel selector here compiles a vertex shader with the
	// wrong texture-coordinate and interpolation behavior, dropping composited
	// layers during affected draws.
