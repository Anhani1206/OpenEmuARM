...
static void ARMSX2MTGSLog(const char* format, ...)
{
...
{
	ARMSX2MTGSLog("[ARMSX2-MTGS] ThreadEntryPoint entered.");
	...
	for (;;)
	{
		...
		ARMSX2MTGSLog("[ARMSX2-MTGS] Calling GSopen.");
		...
		ARMSX2MTGSLog("[ARMSX2-MTGS] GSopen returned %d.", static_cast<int>(opened));
		s_open_flag.store(opened, std::memory_order_release);

		// notify emu thread that we finished opening (or failed)
		ARMSX2MTGSLog("[ARMSX2-MTGS] Posting open result.");
		s_open_or_close_done.Post();
		ARMSX2MTGSLog("[ARMSX2-MTGS] Posted open result.");
...
{
	ARMSX2MTGSLog("[ARMSX2-MTGS] WaitForOpen entered: is_open=%d.", static_cast<int>(IsOpen()));
	if (IsOpen())
		return true;
	...
	ARMSX2MTGSLog("[ARMSX2-MTGS] Requesting open.");
	s_open_flag.store(true, std::memory_order_release);
	s_sem_event.NotifyOfWork();

	// wait for it to finish its stuff
	ARMSX2MTGSLog("[ARMSX2-MTGS] Waiting for open result.");
	s_open_or_close_done.Wait();
	ARMSX2MTGSLog("[ARMSX2-MTGS] Open wait returned.");

	// did we succeed?
	const bool result = s_open_flag.load(std::memory_order_acquire);
	ARMSX2MTGSLog("[ARMSX2-MTGS] WaitForOpen returning %d.", static_cast<int>(result));
	if (!result)
