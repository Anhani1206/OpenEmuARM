...
for entry in "${ZIPS[@]}"; do
  IFS='|' read -r SCHEME PLUGIN VERSION APPCAST ZIP SIZE <<< "$entry"
  DOWNLOAD_URL="https://github.com/Anhani1206/OpenEmuARM/releases/download/${CORES_TAG}/${PLUGIN}.oecoreplugin.zip"
...
echo "╚══════════════════════════════════════════════════════╝"
echo ""
echo "  Draft: https://github.com/Anhani1206/OpenEmuARM/releases/tag/$CORES_TAG"
echo "  Appcasts committed and pushed to main."
echo ""
echo "  When ready to publish:"
echo "  → gh release edit $CORES_TAG --draft=false --repo Anhani1206/OpenEmuARM"
echo ""
