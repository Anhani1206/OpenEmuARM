...
static void ARMSX2GSOpenLog(const char* format, ...)
{
	...
	GSVSyncMode vsync_mode, bool allow_present_throttle)
{
	...
	ARMSX2GSOpenLog("[ARMSX2-GS] OpenGSDevice start: renderer=%d api=%d.", static_cast<int>(renderer), static_cast<int>(new_api));
	switch (new_api)
	{
		case RenderAPI::None:
	...
	ARMSX2GSOpenLog("[ARMSX2-GS] Calling GSDevice::Create.");
	bool okay = g_gs_device->Create(vsync_mode, allow_present_throttle);
	ARMSX2GSOpenLog("[ARMSX2-GS] GSDevice::Create returned %d.", static_cast<int>(okay));
	if (okay)
	{
#ifdef __APPLE__
		const bool openemu_metal = (new_api == RenderAPI::Metal && armsx2_openemu_get_metal_device());
#else
		const bool openemu_metal = false;
#endif
		if (openemu_metal)
		{
			ARMSX2GSOpenLog("[ARMSX2-GS] Skipping ImGuiManager::Initialize for OpenEmu Metal.");
		}
		else
		{
			ARMSX2GSOpenLog("[ARMSX2-GS] Calling ImGuiManager::Initialize.");
			okay = ImGuiManager::Initialize();
			ARMSX2GSOpenLog("[ARMSX2-GS] ImGuiManager::Initialize returned %d.", static_cast<int>(okay));
			if (!okay)
	...
	ARMSX2GSOpenLog("[ARMSX2-GS] OpenGSDevice returning true.");
	return true;
}
...
{
	ARMSX2GSOpenLog("[ARMSX2-GS] OpenGSRenderer start: renderer=%d.", static_cast<int>(renderer));
	// Must be done first, initialization routines in GSState use GSIsHardwareRenderer().
	GSCurrentRenderer = renderer;

	ARMSX2GSOpenLog("[ARMSX2-GS] Calling GSVertexSW::InitStatic.");
	GSVertexSW::InitStatic();
	ARMSX2GSOpenLog("[ARMSX2-GS] GSVertexSW::InitStatic returned.");

	if (renderer == GSRendererType::Null)
	{
		ARMSX2GSOpenLog("[ARMSX2-GS] Creating GSRendererNull.");
		g_gs_renderer = std::make_unique<GSRendererNull>();
	}
	else if (renderer != GSRendererType::SW)
	{
		ARMSX2GSOpenLog("[ARMSX2-GS] Calling GSClampUpscaleMultiplier.");
		GSClampUpscaleMultiplier(GSConfig);
		ARMSX2GSOpenLog("[ARMSX2-GS] Creating GSRendererHW.");
		g_gs_renderer = std::make_unique<GSRendererHW>();
		ARMSX2GSOpenLog("[ARMSX2-GS] GSRendererHW created.");
	}
	else
	{
		ARMSX2GSOpenLog("[ARMSX2-GS] Creating GSRendererSW.");
		g_gs_renderer = std::unique_ptr<GSRenderer>(MULTI_ISA_SELECT(makeGSRendererSW)(GSConfig.SWExtraThreads));
		ARMSX2GSOpenLog("[ARMSX2-GS] GSRendererSW created.");
	}

	ARMSX2GSOpenLog("[ARMSX2-GS] Setting renderer registers.");
	g_gs_renderer->SetRegsMem(basemem);
	g_gs_renderer->ResetPCRTC();
	g_gs_renderer->UpdateRenderFixes();
	ARMSX2GSOpenLog("[ARMSX2-GS] Renderer registers initialized.");
	...
	g_perfmon.Reset();
	ARMSX2GSOpenLog("[ARMSX2-GS] OpenGSRenderer returning true.");
	return true;
}
...
bool GSopen(const Pcsx2Config::GSOptions& config, GSRendererType renderer, u8* basemem,
	GSVSyncMode vsync_mode, bool allow_present_throttle)
{
	ARMSX2GSOpenLog("[ARMSX2-GS] GSopen start: renderer=%d.", static_cast<int>(renderer));
	GSConfig = config;

	if (renderer == GSRendererType::Auto)
		renderer = GSUtil::GetPreferredRenderer();
	ARMSX2GSOpenLog("[ARMSX2-GS] GSopen resolved renderer=%d.", static_cast<int>(renderer));

	bool res = OpenGSDevice(renderer, true, false, vsync_mode, allow_present_throttle);
	ARMSX2GSOpenLog("[ARMSX2-GS] OpenGSDevice returned %d.", static_cast<int>(res));
	if (res)
	{
#ifdef __ANDROID__
		ApplyAndroidGameDBOverrides();
		...
		ARMSX2GSOpenLog("[ARMSX2-GS] OpenGSRenderer returned %d.", static_cast<int>(res));
		if (!res)
			CloseGSDevice(true);
	}
	...
	ARMSX2GSOpenLog("[ARMSX2-GS] GSopen returning true.");
	return true;
}
